...
import dev.supergrecko.kllvm.values.constants.*
import dev.supergrecko.kllvm.values.instructions.Instruction
import org.bytedeco.javacpp.SizeTPointer
import org.bytedeco.llvm.LLVM.LLVMValueRef
import org.bytedeco.llvm.global.LLVM

public open class Value internal constructor() : ContainsReference<LLVMValueRef> {
    ...
    fun constPointerCast(toType: PointerType): PointerValue {
        return PointerValue(
            LLVM.LLVMConstPointerCast(ref, toType.ref)
        )
    }
    ...
    //region Typecasting
    public fun asArrayValue(): ConstantArray =
        ConstantArray(ref)

    public fun asFloatValue(): ConstantFloat =
        ConstantFloat(ref)

    public fun asFunctionValue(): FunctionValue =
        FunctionValue(ref)

    public fun asGenericValue(): GenericValue =
        GenericValue(ref)

    public fun asGlobalValue(): GlobalValue =
        GlobalValue(ref)

    public fun asInstructionValue(): Instruction =
        Instruction(ref)

    public fun asIntValue() =
        ConstantInt(ref)

    public fun asMetadataValue(): MetadataValue =
        MetadataValue(ref)

    public fun asPhiValue(): PhiValue =
        PhiValue(ref)

    public fun asPointerValue(): PointerValue =
        PointerValue(ref)

    public fun asStructValue(): ConstantStruct =
        ConstantStruct(ref)

    public fun asVectorValue(): ConstantVector =
        ConstantVector(ref)
